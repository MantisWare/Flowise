...
import { swaggerDocs, swaggerExplorerOptions } from './api-documentation/configs/swagger.config'
    ...
    async config() {
        ...
        this.app.use(async (req, res, next) => {
            ...
            if (URL_CASE_INSENSITIVE_REGEX.test(req.path)) {
                ...
                // If the req path does not contain /api/v1, then allow the request to pass through, example: /api-docs
                next()
            }
        })
        ...
        this.app.use('/api-docs', swaggerUi.serve, swaggerUi.setup(swaggerDocs, swaggerExplorerOptions))
        ...
        this.app.get('/health', (req: Request, res: Response) => {
            res.json({ status: 'ok', service: 'flowise-runner' })
        })
...
export async function start(): Promise<void> {
    ...
    const port = parseInt(process.env.FLOWISE_RUNNER_PORT || process.env.PORT || '', 10) || 3001
    ...
    server.listen(port, host, () => {
        logger.info(`⚡️ [server]: Flowise Runner is listening at ${host ? 'http://' + host : ''}:${port}`)
        logger.info(`📚 [server]: Swagger API documentation available at http://${host || 'localhost'}:${port}/api-docs`)
    })
}
